...
- **Phase 3.3 active (frontend visualization):** 3.3.A done — `Sparkline` (custom 30-line SVG; explicit dimensions, slate-700 stroke, no axes/legend) + Trend column in `ReportRenderer`'s claims table. 3.3.B (`SectionChart` for featured per-section metrics via Recharts) and 3.3.C (`PeerScatter` for the Peers section) follow. Bundle stays under 100 KB gz budget (currently 75 KB).
