...
- **Phase 3.3 done (frontend visualization):** 3.3.A done (PR #41) — `Sparkline` (custom 30-line SVG) + Trend column. 3.3.B done (PR #42) — `SectionChart` (Recharts LineChart) at the top of Earnings / Quality / Capital Allocation / Macro sections via `featured-claim.ts`. 3.3.C done — `PeerScatter` (Recharts ScatterChart) for the Peers section with subject (Valuation+Quality cross-section join), peers, and median dots. Main bundle 76.92 KB gz (under budget); recharts hoisted into a shared 100 KB chunk shared by both `SectionChart` (4.84 KB) + `PeerScatter` (6.03 KB) lazy chunks. Up next: 3.4 (eval rubric history extension) + 3.5 (Vercel deploy + dogfood gate).
