...
- **Phase 4 active (symbol-centric dashboard rebuild):** Per [ADR 0005](docs/adr/0005-symbol-centric-dashboard.md). 8–10 weeks across 4.0–4.8.
  - **4.0 done:** Strata token system (slate base + 9 category accents + 4 state colors via Tailwind extend), Inter + JetBrains Mono fonts, `react-router-dom@6` routing (`/`, `/symbol/:ticker`, `/login`), `SidebarShell` (72px left rail), `RequireAuth` route guard, `LandingPage` (search bar + recent reports), `SymbolDetailPage` (URL-driven dashboard with hero placeholder above the existing ReportRenderer). Existing 3.3 components (Sparkline, ConfidenceBadge, ReportRenderer, etc.) restyled for dark theme. `Dashboard` and `ReportForm` deleted. Main bundle 83.84 KB gz. 111/111 frontend tests pass.
  - **4.1 next:** Hero card (price chart + 3 featured stats) + Earnings card. Backend addition: `GET /v1/market/:ticker/prices?range=60D`.
  - 4.2–4.7 follow per `tasks/todo.md`. 4.8 = Vercel deploy + dogfood gate.
